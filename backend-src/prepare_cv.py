...
def _convert(f_path):
    if(not os.path.exists(f_path + '.npy')):
        ...
        np.save(f_path[:-3] + '.npy', samples)

_vconvert = np.vectorize(_convert)

def prepare_cv():
    train = pd.read_csv(os.path.join('', 'cv-valid-train.csv'))
    test = pd.read_csv(os.path.join('', 'cv-valid-test.csv'))
    dev = pd.read_csv(os.path.join('', 'cv-valid-dev.csv'))
