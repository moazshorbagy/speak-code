...
def get_features(signal):
    ...
    frames              = framing(emphasized_signal,c.sample_rate,c.frame_duration,c.frame_overlap,np.hanning)
    pow_spec,freq_axis  = pow_spectrum(frames,N=512)
    fbanks              = filter_banks(c.sample_rate,freq_axis)
    mfcc                = np.dot(pow_spec,fbanks.T)
    mfcc                = np.where(mfcc==0,np.finfo(np.float).eps,mfcc)  # replace 0 with small value for computional stability
    mfcc                = 20*np.log10(mfcc)
    mfcc                = scipy.fftpack.dct(mfcc, type=2, axis=1, norm='ortho')
    mfcc               -= (np.mean(mfcc, axis=0) + 1e-8)
    mfcc                = mfcc[:,0:c.n_mfcc]

    # this part add context frames to the mfcc features
    if c.add_context:
        if c.with_deltas:
            dummy_array = np.zeros((c.n_context,3*c.n_mfcc),dtype=mfcc.dtype)
            ...
            mfcc        = np.lib.stride_tricks.as_strided(  mfcc_temp,
                                                            ...
                                                            strides=(3*c.n_mfcc*mfcc.itemsize,mfcc.itemsize))
        else:
...
def framing(signal,sample_rate,frame_duration,frame_overlap,window_func):
    ...
    start_indices       = np.arange(samples_per_frame-int(frame_overlap*sample_rate),len(signal),int(sample_rate*(frame_duration-frame_overlap)))
...
def filter_banks(sample_rate,frequency_axis):
    ...
    Melfreq_axis    = np.floor(700*((10**(mel_axis/2595))-1))
...
def apply_lift(mfcc):
    ...
    :parameter mfcc   : 2D array of mfcc features
    :return    mfcc   : lifted mfcc

    """
    (nframes, ncoeff) = mfcc.shape
    n = np.arange(ncoeff)
    lift = 1 + ((23 / 2) * np.sin(np.pi * n / 23))
    mfcc *= lift
    return mfcc
...
def delta(features, N):
    ...
    :parameter features : 2D array of features to compute delta for.
    :parameter N        : Number of frames used in delta calculations.

    :return    Fdelta   : delta features for features array

    """
    Fdelta = np.empty_like(features)
    denominator = sum([i**2 for i in range(1, N+1)])*2
    padded = np.pad(features, ((N, N), (0, 0)), mode='edge')
    temp   = np.arange(-N,N+1)
    for t in range(len(features)):
        Fdelta[t] = np.dot(temp, padded[t:t+2*N+1])/denominator
    return Fdelta
